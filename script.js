...
async function generateSmartQuestions(apiKey) {
    const prompt = `Given the following context: "${state.context}", generate 2-3 concise, simple and short relevant questions using less than 15 words that a new teacher in the Science Department might ask based on the given context. Do NOT suggest any questions about the Chairperson, the Managing director, the Principal and XLCRSCI-ArchGPT 1.0. Return only the questions as a plain list, one per line, with no extra text or numbering.`;
    try {
